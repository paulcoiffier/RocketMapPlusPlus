...
Please do not open a GitHub issue for support or questions. They are best answered via [Discord](https://discordapp.com/invite/ZZwzc4h). GitHub issues are for RocketMap code issues *only*.
...
* Any 3rd party project that uses RM users' websites needs to be opt-in for the website's owner. Any website that hasn't explicitly opted in should be blocked in your project. A common way of adding an opt-in is to host a file *"myProjectName.txt"* similar to [robots.txt](http://www.robotstxt.org/) that explicitly allows the website to be used in the project.
