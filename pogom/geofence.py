...
class Geofences:
    ...
    def get_geofenced_results(self, list_to_check, name=""):
        ...
        geofences_to_search_for = name.split(",")
        log.info('Requested number of geofences: %d.', len(geofences_to_search_for))
        ...
        if isinstance(list_to_check, dict):
            ...
            for key, item in list_to_check.items():
                ...
                if self.geofenced_areas:
                    for va in self.geofenced_areas:
                        if (name == "" or va["name"] in geofences_to_search_for) and self._in_area(c, va):
