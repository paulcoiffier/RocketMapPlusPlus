...
from .models import (parse_map, GymDetails, parse_gyms, PokestopDetails, parse_pokestop,
                     MainWorker, WorkerStatus, ScannedLocation, Weather)
from .utils import now, get_args, clear_dict_response, distance, degrees_to_cardinal
from .account import (setup_mrmime_account, get_account, account_failed,
                      account_revive)
...
def print_account_stats(rows, thread_status, account_queue,
    ...
    # Print table header.
    row_tmpl = '{:7} | {:' + str(userlen) + '} | {:4} | {:10} | {:3} | {:2} | {:>8} | {:17} | {:14}' \
                                            '| {:>10}'
...
def search_overseer_thread(args, new_location_queue, control_flags, heartb,
    ...
    for i in range(0, args.workers):
        ...
        argset = (
            args, control_flags, threadStatus[workerId],
...
def search_worker_thread(args, control_flags, status, dbq, whq,
                         scheduler, gym_cache, pokestop_cache):
                    ...
                    if len(pokestops_to_update):
                        ...
                        status['message'] = (
                            'Updating {} pokestops for location {},{}...').format(
                                len(pokestops_to_update), scan_coords[0],
                                scan_coords[1])
                        log.debug(status['message'])
                        ...
                        for pokestop in pokestops_to_update.values():
                            status['message'] = (
                                ...
                                'location {:6f},{:6f}...').format(
                                    ...
                                    scan_coords[0], scan_coords[1])
                            time.sleep(random.random() + 2)
                            response = pokestop_request(pgacc, scan_coords, pokestop)
